...
import numpy as np
import torch
####### Model Credit: R2NET by CAO

def _make_triple(value):
...
def conv_layer(in_channels,
               ...
               rep='plain'):
    if rep == 'plain':
        kernel_size = _make_triple(kernel_size)
...
class PixelShuffle3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, out_channels, r, r, r,
...
class ESA(nn.Module):
    def __init__(self, esa_channels, n_feats, conv, bias=True, rep='plain'):
        ...
        self.conv2 = nn.Conv3d(f, f, kernel_size=3, stride=2, padding=0, bias=bias)
    ...
    def forward(self, x):
        c1_ = (self.conv1(x))
        c1 = self.conv2(c1_)
        c1 = F.max_pool3d(c1, kernel_size=7, stride=3)
        c1 = self.conv3(c1)
        c1 = F.interpolate(c1, (x.size(2), x.size(3), x.size(4)), 
